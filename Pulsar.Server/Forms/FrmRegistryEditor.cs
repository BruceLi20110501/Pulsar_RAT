...
namespace Pulsar.Server.Forms
{
    public partial class FrmRegistryEditor : Form
    {
        ...
        private void FrmRegistryEditor_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "客户端软件未以管理员身份运行，因此部分功能（如更新、创建、打开和删除）可能无法正常工作！",
        ...
        private void tvRegistryDirectory_AfterLabelEdit(object sender, NodeLabelEditEventArgs e)
        {
            if (e.Label != null)
            {
                ...
                if (e.Label.Length > 0)
                {
                    ...
                    {
                        MessageBox.Show("无效的标签。\n已存在同名节点。", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        e.Node.BeginEdit();
                    }
                    else
                    {
        ...
        private void deleteRegistryKey_Click(object sender, EventArgs e)
        {
            ...
            string msg = "确定要永久删除此项及其所有子项吗？";
            string caption = "确认删除项";
        ...
        {
            //Prompt user to confirm delete
            string msg = "删除某些注册表值可能导致系统不稳定。确定要永久删除" + (lstRegistryValues.SelectedItems.Count == 1 ? "此值吗？" : "这些值吗？");
            string caption = "确认删除值";
            var answer = MessageBox.Show(msg, caption, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
