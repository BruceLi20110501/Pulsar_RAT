            ...
            try
            {
                switch (message.Action)
                {
                    case ShutdownAction.Shutdown:
                        client.Send(new SetStatus { Message = "客户端正在关机..." });
                    ...
                    case ShutdownAction.Restart:
                        client.Send(new SetStatus { Message = "客户端正在重启..." });
                    ...
                    case ShutdownAction.Standby:
                        client.Send(new SetStatus { Message = "客户端正在进入待机模式..." });
                        if (!SetSuspendState(false, true, true))
                            client.Send(new SetStatus { Message = "待机请求失败。" });
                        break;

                    case ShutdownAction.Lockscreen:
                        client.Send(new SetStatus { Message = "客户端屏幕正在锁定..." });
                        if (!LockWorkStation())
                            client.Send(new SetStatus { Message = "LockWorkStation 失败，无可用回退方案。" });
                        break;

                    default:
                        client.Send(new SetStatus { Message = "请求了未知的关机操作。" });
                        break;
                }
            }
            catch (Exception ex)
            {
                client.Send(new SetStatus { Message = $"关机操作失败: {ex.Message}" });
            }
        }
